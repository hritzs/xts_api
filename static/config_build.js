...
function enforceEntryCutoff() {
    ...
    const exitInput = getEl('config-exit-time');
    ...
    const entrySec = parseTimeToSeconds(entryValue);

    if (entrySec === null) {
        if (typeof showNotification === 'function') {
            showNotification('Invalid entry time.', 'error');
        }
        return false;
    }

    if (exitInput && exitInput.value) {
        const exitSec = parseTimeToSeconds(exitInput.value);

        if (exitSec !== null && entrySec >= exitSec) {
            if (typeof showNotification === 'function') {
                showNotification(
...
function renderLutFactorFields(score) {
    if (!score) {
        return;
    }
    ...
    put(
        'lut-score-live-iv-decimal',
        score.live_iv,
        v => formatNumber(v / 100.0, 6)
    );
    ...
    put(
        'lut-score-build-iv-bucket',
        payload.Build_IV
    );

    put(
        'lut-score-live-straddle',
        score.live_straddle,
        v => formatNumber(v, 2)
    );
    ...
    put(
        'lut-score-dte-bucket',
        payload.DTE
    );

    put(
        'lut-score-prev-day-adj-iv',
        score.prev_day_adj_iv,
        v => formatNumber(v, 6)
    );

    put(
        'lut-score-prev-straddle',
        score.prev_day_straddle,
        v => formatNumber(v, 2)
    );

    put(
        'lut-score-iv-idv-ratio',
        score.iv_idv_ratio,
        v => formatNumber(v, 4)
    );

    put(
        'lut-score-iv-ratio-bucket',
        payload.IV_Ratio
    );

    put(
        'lut-score-straddle-ratio',
        score.straddle_ratio,
        v => formatNumber(v, 4)
    );

    put(
        'lut-score-straddle-ratio-bucket',
        payload.Straddle_Ratio
    );

    put(
        'lut-score-adj-iv-chg',
        score.adj_iv_chg,
        v => formatNumber(v, 6)
    );

    put(
        'lut-score-adj-iv-chg-bucket',
        payload.Adj_IV_Chg
    );

    put(
        'lut-score-future-price-ref',
        score.future_price_ref,
        v => formatNumber(v, 2)
    );

    put(
        'lut-score-synthetic-price-ref',
        score.synthetic_price_ref,
        v => formatNumber(v, 2)
    );

    put(
        'lut-score-price-ref-source',
        score.price_ref_source
    );

    put(
        'lut-score-og-gap-pct',
        score.og_gap_pct,
        v => `${(v * 100).toFixed(2)}%`
    );

    put(
        'lut-score-norm-og-gap',
        score.norm_og_gap,
        v => formatNumber(v, 6)
    );

    put(
        'lut-score-norm-og-gap-bucket',
        payload.Norm_OG_Gap
    );

    const bannerInfo =
        typeof buildScoreBannerMessage === 'function'
            ? buildScoreBannerMessage(score)
            : null;

    const banner =
        document.getElementById(
            'lut-score-status-banner'
        );

    if (banner && bannerInfo) {
        banner.textContent =
            bannerInfo.message || 'Waiting for score...';
    }
}
...
function getUiConfig() {
    ...
    return {
        ...
        lut_based_selling: !!window.__LUT_STAGE_MODE__,
...
// ============================================================
// FIVE-STAGE LUT SELLING RUNTIME
// ============================================================

window.__LUT_STAGE_MODE__ = false;
window.__LUT_STAGE_ACTIVE__ = false;

function setLutStageStatus(id, text) {
    const el = document.getElementById(id);
    if (el) el.textContent = text;
}

function getLutStageConfig() {
    return {
        symbol:
            document.getElementById('lut-stage-symbol')?.value ||
            getEl('config-symbol')?.value ||
            'NIFTY',

        size: Number(
            document.getElementById('lut-stage-size')?.value ||
            getEl('config-size')?.value ||
            77
        ),

        exit_time:
            document.getElementById('lut-stage-exit-time')?.value ||
            getEl('config-exit-time')?.value ||
            '15:27:00'
    };
}

function resetLutStageUI() {
    [
        ['lut-stage-0916', 'WAITING'],
        ['lut-stage-0917', 'WAITING'],
        ['lut-stage-0918', 'WAITING'],
        ['lut-stage-0919', 'WAITING'],
        ['lut-stage-0920', 'WAITING'],
    ].forEach(([id, value]) => setLutStageStatus(id, value));

    const status = document.getElementById('lut-stage-status');

    if (status) {
        status.textContent = 'Five-stage LUT engine ready.';
    }
}

function markCurrentLutStage(timeText, resultText) {
    if (!timeText) return;

    const h = Number(timeText.slice(0, 2));
    const m = Number(timeText.slice(3, 5));
    ...
    if (id) {
        setLutStageStatus(id, resultText);
    }
}

async function startFiveStageLutSelling() {
    if (window.__LUT_STAGE_ACTIVE__) {
        showNotification?.(
            'Five-stage LUT selling is already running.',
            'warning'
        );
        return;
    }

    const cfg = getLutStageConfig();

    const exitSec = parseTimeToSeconds(cfg.exit_time);

    if (exitSec === null) {
        showNotification?.(
            'Invalid LUT exit time.',
            'error'
        );
        return;
    }

    window.__LUT_STAGE_MODE__ = true;
    window.__LUT_STAGE_ACTIVE__ = true;
    ...
    if (status) {
        status.textContent =
            'Five-stage LUT selling started. Waiting for 09:16 stage.';
    }

    try {
        // Reuse the existing config build endpoint.
        // Backend recognizes lut_based_selling=true and ignores
        // entry_time as a trigger.
        const payload = getUiConfig();

        payload.symbol = cfg.symbol;
        payload.size = cfg.size;

        // Required by current schema/backend, but ignored by the
        // dedicated five-stage mode.
        payload.entry_time = '09:16:00';
        payload.exit_time = cfg.exit_time;

        payload.lut_based_selling = true;

        const response = await fetch(
            '/api/straddle/config-build',
            {
                method: 'POST',
                headers: {
        ...
        if (status) {
            status.textContent =
                'Five-stage LUT selling scheduled successfully.';
        }

        showNotification?.(
            'Five-stage LUT selling started.',
            'success'
        );

    } catch (error) {
        console.error(
            'Five-stage LUT selling failed:',
            error
        );
        ...
        showNotification?.(
            `LUT selling failed: ${error.message}`,
            'error'
        );
    }
}

function stopFiveStageLutSelling() {
    window.__LUT_STAGE_ACTIVE__ = false;
    window.__LUT_STAGE_MODE__ = false;
    ...
    if (status) {
        status.textContent =
            'Stop requested. Backend cancellation must be handled by the active trade/task.';
    }

    showNotification?.(
        'LUT selling stop requested.',
        'warning'
    );
}
...
document.addEventListener('DOMContentLoaded', () => {
    const startBtn = document.getElementById(
        'start-lut-stage-build'
    );

    const stopBtn = document.getElementById(
        'stop-lut-stage-build'
    );

    if (startBtn) {
        startBtn.addEventListener(
            'click',
            startFiveStageLutSelling
        );
    }

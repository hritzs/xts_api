...
function displayPortfolioPositions(straddles) {
    ...
    straddles.forEach(straddle => {
        ...
        const ce_ltp_display = isClosed || isPending
            ? '—' : `₹${(priceMap[straddle.ce_token] || straddle.ce_ltp || 0).toFixed(2)}`;
        const pe_ltp_display = isClosed || isPending
            ? '—' : `₹${(priceMap[straddle.pe_token] || straddle.pe_ltp || 0).toFixed(2)}`;
...
function handleStraddleUpdate(data) {
    ...
    if (mainRow) {
        ...
        if (deltaCell) deltaCell.textContent =
            (data.live_net_delta || data.net_delta || 0).toFixed(2);

        const pnlCell = mainRow.querySelector('.pnl-cell');
        if (pnlCell) {
            const pnl = data.live_pnl ?? data.total_pnl ?? 0;
            pnlCell.textContent = `₹${pnl.toFixed(2)}`;
            pnlCell.className   = `pnl-cell ${pnl >= 0 ? 'positive' : 'negative'}`;
        }
    ...
    if (portfolioRow) {
        ...
        if (strikeCell && data.strike) strikeCell.textContent = data.strike;
            ...
            if (data.status !== prevStatus) {
                ...
                if (actionsCell)
                    actionsCell.innerHTML = _buildActionsHtml(
                        tradeUid, data.status, data.live_net_delta || data.net_delta || 0
                    );
            }
        }
        ...
        if (ptsOutCell) {
            const ptsOut        = data.pts_out ?? 0;
            const pointsAllowed = data.points_allowed ?? null;
            ptsOutCell.textContent = ptsOut.toFixed(2);

            let cls = 'positive';
            if (pointsAllowed !== null && pointsAllowed > 0) {
                const ratio = ptsOut / pointsAllowed;
                cls = ratio >= 1.0 ? 'negative' : ratio >= 0.5 ? 'yellow' : 'positive';
            }
            ptsOutCell.className = `pts-out-cell ${cls}`;
        }
        ...
        if (ptsAllowedCell) {
            ...
            ptsAllowedCell.textContent =
                (pa !== null && pa !== undefined) ? pa.toFixed(2) : '∞';
        }
    ...
    // Update spot/synfut header spans (never overwrite label text)
    if (data.spot_price) {
        const el = document.getElementById('chain-spot-value');
        if (el) el.textContent = `₹${data.spot_price.toFixed(2)}`;
    }
    if (data.synthetic_spot) {
        const el = document.getElementById('chain-synfut-value');
        if (el) el.textContent = `₹${data.synthetic_spot.toFixed(2)}`;
    }
}
...
async function sellStraddle() {
    ...
    try {
        ...
        if (result.success) {
            ...
            setTimeout(() => fetchStraddles(false), 2000);
            setTimeout(() => fetchStraddles(true),  2000);
        } else {

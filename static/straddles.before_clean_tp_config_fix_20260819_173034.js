...
function handleStraddleUpdate(data) {
    ...
    if (typeof renderLutFactorFields === 'function') {
...
async function showStraddleDetails(tradeUid, clickedRow) {
    ...
    try {
        ...
        // ====================================================
        // STEP 1:
        // GET THE CANONICAL TRADE LIST FIRST.
        //
        // This is the important part.
        // Position quantities already available in this
        // response must NEVER be discarded just because the
        // optional snapshot request fails.
        // ====================================================

        const straddlesResponse =
            await fetch(
                `/api/straddles?ts=${Date.now()}`,
                {
                    cache: 'no-store'
                }
            );

        if (!straddlesResponse.ok) {

            throw new Error(
                `Trade list failed with status ${straddlesResponse.status}`
            );
        }

        const straddlesData =
            await straddlesResponse.json();

        if (!straddlesData.success) {

            throw new Error(
                'Failed to fetch base straddles data'
            );
        }

        const straddles =
            Array.isArray(
                straddlesData.straddles
            )
                ? straddlesData.straddles
                : [];
        ...
        if (!baseStraddle) {
            ...
            throw new Error(
                `Base data for ${tradeUid} not found`
            );
        }

        // ====================================================
        // STEP 2:
        // NORMALIZE POSITION DATA ALREADY PRESENT IN THE
        // TRADE OBJECT.
        // ====================================================

        let baseLivePositions = [];

        if (
            Array.isArray(
                baseStraddle.live_positions
            )
        ) {
        ...
        // ====================================================
        // STEP 3:
        // IF THE TRADE OBJECT HAS NO POSITION ARRAY BUT HAS
        // DIRECT CE/PE QUANTITIES, BUILD THE TWO LEG OBJECTS.
        //
        // This is only a UI fallback.
        // It does NOT change trading/accounting logic.
        // ====================================================

        if (
            baseLivePositions.length === 0
        ) {

            const ceQty = Number(
                baseStraddle.ce_qty
                ??
                baseStraddle.ce_quantity
                ??
                0
            );
        ...
        // ====================================================
        // STEP 4:
        // OPTIONAL FRESH SNAPSHOT.
        //
        // IMPORTANT:
        // Correct endpoint is /api/snapshots/<uid>.
        //
        // If snapshot fails, DO NOT wipe positions.
        // ====================================================
        ...
        try {
            ...
            const snapshotResponse =
                await fetch(
                    `/api/snapshots/${encodeURIComponent(tradeUid)}?ts=${Date.now()}`,
                    {
                        cache: 'no-store'
                    }
                );
            ...
            if (
                ...
                console.warn(
                    `Snapshot unavailable for ${tradeUid}: HTTP ${snapshotResponse.status}. ` +
                    `Using trade-list position data.`
                );
            }
            ...
            console.warn(
                `Snapshot request failed for ${tradeUid}. ` +
                `Using trade-list position data.`,
                snapshotError
            );
        }
        ...
        // ====================================================
        // STEP 5:
        // ONLY REPLACE LIVE POSITIONS WHEN SNAPSHOT ACTUALLY
        // CONTAINS THEM.
        //
        // An empty snapshot array is NOT allowed to erase
        // known trade positions.
        // ====================================================

        let finalLivePositions =
            baseLivePositions;

        if (
            Array.isArray(
                snapshotData.live_positions
            )
            &&
            snapshotData.live_positions.length > 0
        ) {
            ...
            &&
            snapshotData.positions.length > 0
        ) {
        ...
        // ====================================================
        // STEP 6:
        // MERGE SNAPSHOT METRICS ON TOP OF THE BASE TRADE.
        // BASE TRADE POSITIONS REMAIN AVAILABLE.
        // ====================================================
        ...
        const straddle = {
            ...
            live_pnl:
                snapshotData.total_pnl
                ??
                snapshotData.live_pnl
                ??
                baseStraddle.live_pnl
                ??
                0,
            ...
            realized_pnl:
                ...
                ??
                baseStraddle.realized_pnl
                ??
                0,

            live_net_delta:
                snapshotData.net_delta
                ??
                baseStraddle.net_delta
                ??
                0,

            pnl_per_lot:
                snapshotData.pnl_per_straddle
                ??
                baseStraddle.pnl_per_straddle
                ??
                0
        };
        ...
        // ====================================================
        // STEP 7:
        // LOG WHAT THE UI ACTUALLY RECEIVED.
        // ====================================================

        console.info(
            `[TRADE DETAILS] ${tradeUid} | ` +
            `base_positions=${baseLivePositions.length} | ` +
            `snapshot_positions=${
                Array.isArray(
                    snapshotData.live_positions
                )
                    ? snapshotData.live_positions.length
                    : 0
            } | ` +
            `final_positions=${finalLivePositions.length}`
        );
        ...
        clickedRow.parentNode.insertBefore(
            detailsRow,
            clickedRow.nextSibling
        );

    } catch (error) {

        console.error(
            `Failed to show details for ${tradeUid}:`,
            error
        );

        showNotification(
            `❌ Could not load details for ${tradeUid}.`,
            'error'
        );

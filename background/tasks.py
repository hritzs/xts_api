...
async def websocket_keepalive_loop():
    """Send periodic pings to keep connections alive via broadcast_message (thread-safe)"""
...
async def verify_orders_task(
    ...
    try:
        ...
        # --- FIX: Handle cases where verification returns empty but orders were pending ---
        if not verified_success and not verified_failed and order_ids:
            logger.warning(f"Verification for {batch_name} returned no results for {len(order_ids)} pending orders. This may be due to an empty order book response. Treating as temporary failure.")
            # Return a list of failures so the calling function knows these are still pending.
            # The calling function's retry logic will handle this.
            verified_failed = [{'order_id': oid, 'status': 'VERIFICATION_PENDING', 'reason': 'Empty or failed order book fetch'} for oid in order_ids]
...
async def update_order_book_loop():
    """Periodically ping order book service to ensure it's alive/caching"""
    ...
    try:
        while True:
            try:
                ...
                pass # Service might be down, just retry later
    except asyncio.CancelledError:
...
async def _create_snapshot_for_trade(trade: dict, spot_details_map: dict, trade_data_override: dict = None, log_level: str = 'DEBUG'):
    ...
    try:
                ...
                if atm_row:
                    ...
                    if ce_token and pe_token:
                        ...
                        if ce_ltp and pe_ltp and ce_ltp > 0 and pe_ltp > 0:
                            # Synthetic Future = Strike + CE - PE (Put-Call Parity)
                            calc_spot = atm_strike + ce_ltp - pe_ltp
                            
                            # Sanity check against cached future to prevent bad data (e.g. cross-symbol collision)
                            cached_fut = option_chain.get('fut_ltp', 0)
                            if calc_spot <= 0:
                                logger.warning(f"⚠️ Live synthetic spot {calc_spot:.2f} is negative. Using cached future {cached_fut:.2f}.")
                                live_spot_price = cached_fut
                            elif cached_fut > 0 and abs(calc_spot - cached_fut) > (cached_fut * 0.05):
                                logger.warning(f"⚠️ Live synthetic spot {calc_spot:.2f} deviates > 5% from cached future {cached_fut:.2f}. Using cached future.")
                                live_spot_price = cached_fut
                            else:
                                live_spot_price = calc_spot
        except Exception:
            pass # Keep fallback
        # --- END NEW ---
        ...
        try:
            ...
            for order in trade_orders:
                ...
                # --- FIX: Use strict check to prevent 'ny...14' matching 'ny...14a' ---
                # We check if the broker UID contains the trade UID followed by an underscore or end of string,
                # or simply if it contains the trade_uid and the lengths imply a correct match context.
                if order_uid_from_broker:
                    if trade_uid in order_uid_from_broker:
                        filtered_trade_orders.append(order)
                    else:
                        logger.warning(f"Skipping order {order.get('AppOrderID')} in snapshot for {trade_uid}: OrderUniqueIdentifier '{order_uid_from_broker}' does not match trade_uid.")
                else:
                    logger.warning(f"Skipping order {order.get('AppOrderID')} in snapshot for {trade_uid}: Missing OrderUniqueIdentifier.")
        ...
        num_straddle_contracts = (abs(net_ce_qty) + abs(net_pe_qty)) / 2.0
        
        # --- FIX: Calculate PnL per LOT, not per contract ---
        num_straddle_lots = num_straddle_contracts / lot_size if lot_size > 0 else 0
        
        # --- NEW: Fallback for hedged positions ---
        # If net lots are very small (e.g. < 1) but we have significant PnL, 
        # it implies a hedged/neutralized position. Use the gross open interest or DB lots to normalize.
        # We check if num_straddle_lots is significantly smaller than the gross lots from DB.
        if num_straddle_lots < 1.0 and abs(total_unrealized_pnl) > 1000:
             # Try to use the gross quantity from DB as a more stable denominator
             db_ce_qty = full_trade_data.get('ce_quantity', 0)
             db_pe_qty = full_trade_data.get('pe_quantity', 0)
             if db_ce_qty > 0 or db_pe_qty > 0:
                 gross_lots_db = (db_ce_qty + db_pe_qty) / (2.0 * lot_size)
                 if gross_lots_db > 0:
                     num_straddle_lots = gross_lots_db
        
        pnl_per_straddle = 0.0
        if num_straddle_lots > 0:
            # --- MODIFIED: Calculate based on UNREALIZED PnL and CURRENT Lots ---
            # This reflects the performance of the currently open position.
            pnl_per_straddle = total_unrealized_pnl / num_straddle_lots
        elif total_pnl != 0: # Position is fully closed
            # For closed trades, it's based on total PnL and the main position size.
            ce_qty = full_trade_data.get('ce_quantity', 0)
            pe_qty = full_trade_data.get('pe_quantity', 0)
            straddle_contracts = (ce_qty + pe_qty) / 2.0
            straddle_lots = straddle_contracts / lot_size if lot_size > 0 else 0
            
            if straddle_lots > 0:
                pnl_per_straddle = total_pnl / straddle_lots
        ...
        # --- FIX: Scale SL points to SL Value per Lot for consistent comparison in builder.py ---
        # Since pnl_per_straddle is now Rupees/Lot, sl_points must also be Rupees/Lot.
        sl_value_per_lot = sl_points_per_straddle * lot_size
        ...
        current_straddles = num_straddle_lots if num_straddle_lots > 0 else 0.0
        # Threshold is Total PnL Limit (Rupees)
        sl_threshold = -1 * sl_value_per_lot * current_straddles
        ...
        snapshot_data = {
            ...
            'sl_threshold': sl_threshold, 'sl_points': sl_value_per_lot, # Send Value/Lot to UI/Builder
...
async def create_snapshot_for_trade(trade_uid: str, trade_data: dict = None, log_level: str = 'DEBUG'):
    ...
    await _create_snapshot_for_trade(trade, spot_details_map, trade_data_override=trade_data, log_level=log_level)

async def trigger_snapshot_and_broadcast(trade_uid: str, trade_data: dict = None, log_level: str = 'INFO'):
    """
...
async def create_trade_snapshots_loop(interval_seconds: float = 0.5):
    ...
    snapshot_service_url = f"http://localhost:{getattr(config, 'SNAPSHOT_SERVICE_PORT', 8003)}/api/push-snapshots"
    ...
    while True:
        try:
            ...
            if lightweight_updates:
                # Try to push to the separate Snapshot Service first
                try:
                    async with httpx.AsyncClient(timeout=0.2) as client:
                        await client.post(snapshot_service_url, json={"updates": lightweight_updates})
                except Exception:
                    # Fallback to local broadcast if service is down or not configured

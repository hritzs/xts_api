    ...
    async def place_single_order(
        ...
        async with self.semaphore:
            ...
            try:
                ...
                future = loop.run_in_executor(None, self._place_order_sync, order_params, exchange_name, option_type, action, token, uid, expected_price, exchange_segment)
                try:
                    # ✅ Set a shorter timeout (7 seconds) for the entire order placement process.
                    response = await asyncio.wait_for(future, timeout=7.0)
                except asyncio.TimeoutError:
                    logger.error(f"❌ Order placement for {uid} timed out after 7 seconds. The broker API call is likely stuck.")
    ...
    async def modify_and_chase_order(self, pending_order_data: Dict, attempt_number: int = 0, trade_uid: str = None) -> Dict:
        ...
        try:
            ...
            if not (depth and depth.get('bid_price', 0) > 0 and depth.get('ask_price', 0) > 0):
                ...
                cancel_response = await loop.run_in_executor(None, cancel_func)
            ...
            response = await loop.run_in_executor(None, mod_func)
    ...
    async def verify_orders_bulk(self, order_ids: List[str], batch_name: str = "VERIFY") -> Dict:
        ...
        try:
            ...
            try:
                ...
                order_book = await loop.run_in_executor(None, order_book_func)
                # --- END FIX ---
            except Exception as e:
            ...
            if pending_orders:
                ...
                try:
                    ...
                    order_book_retry = await loop.run_in_executor(None, order_book_retry_func)
                    # --- END FIX ---
...
def set_order_executor(xt_interactive, max_concurrent: int = 50, client_id: str = None):

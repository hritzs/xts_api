...
async def build_with_config(config: Dict, trade_uid: str = None) -> Optional[str]:
    ...
    Args:
        config: Build configuration dict with:
            ...
            - hedge params
            - sl_bps
        trade_uid (Optional): Pre-generated trade UID. If not provided, one will be created.
    
    Returns:
        Trade UID if built, None otherwise
    """
    try:
        # Parse times
        ...
        # --- NEW: Define a pre-entry check window ---
        PRE_ENTRY_CHECK_SECONDS = 15  # Start checking 15 seconds before entry time
        ...
        # --- REFACTORED: Single loop for waiting and checking ---
        while True:
            now_dt = datetime.now()
            ...
            if now_time >= exit_time:
                logger.warning(f"⏰ Exit time reached, aborting build.")
                return None

            # 2. Check if we are in the active period (pre-check window or later)
            if now_time < pre_entry_check_time:
                # Not yet time to check, sleep until the pre-check window starts
                sleep_until = datetime.combine(today, pre_entry_check_time)
            ...
            # --- NEW: Build logic is now INSIDE the loop ---
            ...
            if filters_passed_at_least_once and now_time >= entry_time:
                ...
                break # Exit the while loop to proceed with building.

            # 3. We are in the active period, perform filter checks
            logger.info(f"🔍 CHECKING ENTRY FILTERS @ {now_time}")
            
            # --- REFACTORED: Get option chain from state cache or service API ---
            ...
            if not chain_data:
                logger.warning("⚠️ Could not get option chain from cache or service. Waiting 10s...")
                await asyncio.sleep(10)
                continue
            
            atm_row = next((row for row in chain_data['chain'] if row['is_atm']), None)
            if not atm_row:
                logger.warning("⚠️ Could not find ATM row in option chain. Waiting 10s...")
                await asyncio.sleep(10)
                continue
            
            # Get current IV (average of CE and PE)
            ce_iv = atm_row.get('ce_iv', 0)
            pe_iv = atm_row.get('pe_iv', 0)
            current_iv = (ce_iv + pe_iv) / 2
            
            # --- REFACTOR: IV and Straddle filter logic moved inline to prevent multiple option chain builds ---
            
            # --- IV FILTER LOGIC (INLINE) ---
            logger.info("="*100)
            ...
            if all_filters_passed:
                logger.info("✅ All entry filters passed. Will now wait for entry time.")
                filters_passed_at_least_once = True
                # Don't break here. Continue the loop to wait for the exact entry time.
                # Sleep for a short interval until the entry time is near.
                if now_time < entry_time:
                    sleep_duration = (entry_datetime - now_dt).total_seconds()
                    # Sleep for a shorter duration as we get closer to the entry time
                    sleep_interval = min(2.0, max(0.1, sleep_duration - 0.5))
                    logger.info(f"✅ Filters passed. Waiting {sleep_interval:.2f}s for entry time {entry_time}...")
                    await asyncio.sleep(sleep_interval)
                # Continue to the next iteration of the while loop
            else:
                # Filters failed.
                sleep_interval = 2 if now_time < entry_time else 60
                logger.info(f"⏳ Filters not passed. Re-checking in {sleep_interval}s.")
                await asyncio.sleep(sleep_interval)

        # --- This part of the code is now executed only after the loop breaks ---
        if not build_triggered:
            return None # Loop exited for other reasons (e.g., exit time)

        # If a trade_uid was NOT provided (e.g., for older flows), generate one.
        # Otherwise, use the one passed from the API route.
        if not trade_uid:
            ...
            prefix = next((SYMBOL_PREFIXES[key] for key in sorted_keys if key in symbol_upper), symbol_upper[:2].lower())
        ...
        result = await build_straddle(
            symbol=config['symbol'], lots=config['size'], trade_uid=trade_uid,
            delta_neutral=True, trade_config=config
        )
        ...
        if result and result.get('success'):
            ...
            # Calculate SL points and update the DB with the final config
            ...
            logger.error(f"❌ Build failed for {trade_uid}")
            return None

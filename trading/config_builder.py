...
async def build_with_config(config: Dict, trade_uid: str = None) -> Optional[str]:
    ...
    try:
        ...
        while True:
            # NEW: Check cancellation flag at start of loop
            ...
            if now_time < entry_time:
                ...
                logger.info(f"  Waiting {sleep_interval:.2f}s for entry time {entry_time}...")
            elif all_filters_passed:
                # Time reached AND filters passed. Loop back immediately to trigger build.
                ...
                # Time reached BUT filters failed.
                sleep_interval = 60
        ...
        if not trade_uid:
            ...
            prefix = next((SYMBOL_PREFIXES[k] for k in sorted_keys if k in symbol_upper),
        ...
        if result and result.get('success'):
            ...
            # ✅ FIX: Check current DB status BEFORE returning None.
            # If builder.py placed orders but verification timed out, status is already
            # PARTIAL in the DB. We must NOT let the caller overwrite it to FAILED_FILTER.
            loop = asyncio.get_event_loop()
            ...
            if current_status == 'PARTIAL':
                ...
                # Signal the caller that this failure is PARTIAL, not a clean miss
                # by storing in state — checked in api/routes.py
            ...
            return None  # Always return None on failure — caller guards status update below

...
async def build_with_config(config: Dict, trade_uid: str = None) -> Optional[str]:
    ...
    try:
        ...
        while True:
            ...
            # ========================================================
            # LUT is NOT a gate for the normal Configuration-Based
            # Build. The dedicated LUT Selling mode owns the
            # five-stage LUT decision logic.
            # ========================================================

            logger.info(
                "✅ NORMAL CONFIG MODE | "
                "Entry time reached. "
                "LUT decision is informational only. "
        ...
        if _is_lut_stage_mode(config) and decision != "YES":

            raise ValueError("LUT rejected trade")

        TEST_HALF_SIZE=False

...
async def extract_positions_from_straddle(straddle_data: Dict) -> List[Dict]:
    ...
    try:
        ...
        for token, pos_data in net_positions.items():
            ...
            pos_data['ltp'] = current_price
            pos_data['net_qty'] = abs(net_qty if 'net_qty' in locals() else pos_data.get('quantity', 0))
            pos_data['qty'] = abs(net_qty if 'net_qty' in locals() else pos_data.get('quantity', 0))
            pos_data['avg_price'] = entry_price if 'entry_price' in locals() else pos_data.get('entry_price', 0.0)
    ...
    try:
        ...
        for pos in positions:
            ...
            if not current_price or current_price <= 0:
                ...
                if not current_price or current_price <= 0: current_price = pos.get('entry_price', 1.0)

...
async def partial_square_off(trade_uid: str, percentage_of_original: float):
    ...
    try:
        ...
        for chunk_idx, chunk_orders in enumerate(all_chunks, 1):
            ...
            if not chunk_orders: psqf_aborted = True; break

            while True:
                exit_target = ((straddle_data.get("config") or {}).get("exit_at_straddle"))
                if await exit_chunk_price_allowed(trade_uid, straddle_data.get("symbol", "NIFTY"), exit_target): break
                await asyncio.sleep(1.0)
        ...
        return {'success': is_successful, 'successful_count': len(all_successful_orders), 'failed_count': len(all_failed_orders), 'execution_time': batch_execution_time, 'trade_uid': trade_uid}

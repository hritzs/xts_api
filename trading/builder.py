        ...
        while total_unfilled_qty > 0 and not build_aborted and sweep_attempt < max_sweep_attempts:
            ...
            if sweep_legs:
                ...
                for chunk in sweep_chunks:
                    ...
                    # ========================================================
                    # MANUAL_ENTRY_PRICE_GUARD_V2 - FINAL SWEEP
                    #
                    # Sweep orders are still BUILD SELL orders.
                    # Therefore the exact same entry_at_straddle condition
                    # must be checked immediately before submission.
                    #
                    # current >= target -> submit
                    # current <  target -> wait and re-check
                    # ========================================================
                    while True:
                        build_price_allowed = await build_chunk_price_allowed(
                            trade_uid=trade_uid,
                        ...
                        if build_price_allowed:
                            logger.info(
                                f"[{trade_uid}] ENTRY PRICE GUARD PASS | "
                                f"Sweep {sweep_attempt} chunk allowed"
                            )
                            break

                        logger.warning(
                            f"[{trade_uid}] ENTRY PRICE GUARD BLOCK | "
                            f"Sweep {sweep_attempt} chunk waiting for target"
                        )
                    ...
                    sweep_result = await executor.execute_batch(
                        chunk,
                        f"W_{trade_uid}_SWEEP{sweep_attempt}"
                    )

...
async def build_with_config(config: Dict, trade_uid: str = None) -> Optional[str]:
    ...
    try:
        ...
        entry_datetime = dt.dt.datetime.combine(today, entry_time, tzinfo=now_dt_init.tzinfo)
        ...
        async def _sleep_until_next_minute_boundary(now_dt: datetime, today_date) -> bool:
        ...
        while True:
            ...
            # --- NEW: Wait until entry time for a single check ---
            if now_dt < entry_datetime:
                sleep_until = entry_datetime
                sleep_duration = max(0.0, (sleep_until - now_dt).total_seconds())
                if sleep_duration > 0:
                    logger.debug(f"Waiting until entry time {entry_time} ({sleep_duration:.1f}s)...")
            ...
            logger.debug(f"  Trade Size    : {final_trade_size}")
            ...
            if sell_allowed:
                logger.debug("✅ LUT check passed. Triggering build.")
                build_triggered = True
                break
            else:
                logger.warning("❌ LUT check failed at entry time. Aborting build.")
                return None

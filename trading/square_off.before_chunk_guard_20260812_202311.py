...
async def extract_positions_from_straddle(straddle_data: Dict) -> List[Dict]:
    ...
    try:
        # --- Fetch ALL of today's orders and filter robustly in Python ---
        ...
        # --- Merge with trade_fill_cache which holds recent, unpersisted fills (e.g., from pre-sqf hedge) ---
        cached_fills = []
        if hasattr(state, 'trade_fill_cache') and state.trade_fill_cache:
            cached_fills = list(state.trade_fill_cache.get(trade_uid, []))
            if cached_fills:
                logger.info(
                    f"Found {len(cached_fills)} orders in trade_fill_cache for {trade_uid}. "
                    f"Merging for square-off."
                )
        elif hasattr(state, 'temp_order_cache') and state.temp_order_cache:
            cached_fills = state.temp_order_cache.get(trade_uid, []) or []
            logger.warning(f"Legacy temp_order_cache fallback used for {trade_uid}.")
        ...
        if cached_fills:
            ...
            for fill in cached_fills:
                app_order_id = str(fill.get('AppOrderID') or fill.get('app_order_id')
                    or fill.get('apporderid')
                    or fill.get('app_orderid')
                )
        ...
        if not trade_orders:
            logger.warning(
                f"⚠️ No filled orders found for trade {trade_uid} in DB or cache. Cannot determine position."
            )
            return []
        ...
        for order in trade_orders:
            ...
            if not token_val:
                order_id_for_log = (
                    ...
                    or order.get('app_order_id')
                    or order.get('order_id')
                    or 'UNKNOWN_ID'
                )
                logger.warning(
                    f"Skipping order {order_id_for_log} in position extraction: missing instrument token."
                )
                continue
            ...
            try:
                ...
                logger.error(
                    f"❌ Invalid token value '{token_val}' for order "
                    f"{order.get('AppOrderID') or order.get('app_order_id')}. "
        ...
        # Re-derive exchange segment from symbol
        base_symbol = next(
        ...
        if derived_segment:
            ...
            logger.warning(
                f"Could not derive segment for {symbol_upper} from SYMBOL_CONFIG. "
                f"Falling back to DB value: {correct_exchange_segment}."
            )

        option_chain = state.option_chains.get(symbol_upper)

        # Persisted metadata map for primary-leg resolution inside worker processes
        ...
        for token, pos_data in net_positions.items():
            ...
            # --- 1) Resolve from persisted trade metadata FIRST ---
            if persisted_strike > 0:
                if token == persisted_ce_token:
                    strike, option_type = persisted_strike, 'CE'
                    logger.info(f"✅ Resolved {token} to {strike} CE from persisted trade metadata.")
                elif token == persisted_pe_token:
                    strike, option_type = persisted_strike, 'PE'
                    logger.info(f"✅ Resolved {token} to {strike} PE from persisted trade metadata.")

            # --- 2) Fallback to live option chain for rolled/extra legs ---
            if not strike and option_chain and option_chain.get('chain'):
                for row in option_chain['chain']:
                    if row.get('ce_token') == token:
                        ...
                        logger.info(f"✅ Resolved {token} to {strike} CE from live option chain.")
                        break
                    if row.get('pe_token') == token:
                        strike, option_type = row['strike'], 'PE'
                        logger.info(f"✅ Resolved {token} to {strike} PE from live option chain.")
                        break

            # --- 3) Final fallback to parsing TradingSymbol from order history ---
            if not strike:
                logger.warning(
                    f"Could not find token {token} in persisted trade metadata or live option chain "
                    f"for {trade_uid}. Falling back to parsing TradingSymbol from order history."
                )
                order_with_token = next(
                    (
                        o for o in trade_orders
                        if int(o.get('exchange_instrument_id') or o.get('ExchangeInstrumentID') or 0) == token
                    ),
                    None
                )
                if order_with_token:
                    ...
                    if trading_symbol:
                        ...
                        if match:
                            ...
                            logger.info(
                                f"✅ Resolved {token} to {strike} {option_type} "
                                f"from TradingSymbol '{trading_symbol}'"
                            )

            if not strike:
                logger.error(
                    f"❌ Could not resolve strike/type for token {token} from persisted trade metadata, "
                    f"chain, or order history. Square-off may fail for this leg."
                )
                strike = 0
                option_type = 'UNKNOWN'
            ...
            if current_price <= 0:
                logger.warning(
                    f"⚠️ Missing current_price for token {token} in extract_positions_from_straddle. "
                    f"Fetching fresh LTP."
                )
                current_price = await get_ltp_from_service(token)
                if current_price <= 0:
                    logger.error(
                        f"❌ Could not fetch valid LTP for token {token} in "
                        f"extract_positions_from_straddle. Price will be 0 for this leg."
                    )
                    current_price = 0.0
    ...
    try:
        ...
        if hasattr(state, 'trade_processes') and trade_uid in state.trade_processes:
            ...
            if not process or not process.is_alive():
                logger.error(f"Process for trade {trade_uid} is not alive. Cannot partial square-off.")
            ...
            if not command_q:
                ...
                return {'success': False, 'error': 'Trade command queue is not available.'}
        ...
        build_orders = [
            ...
            if trade_uid in (o.get('OrderUniqueIdentifier') or o.get('order_unique_id', ''))
        ...
        sqf_ratio = (target_sqf_qty / current_total_qty) if current_total_qty > 0 else 0.0
        ...
        for pos in positions:
            ...
            if lots_to_sqf > 0:
                legs_data_for_batching.append({
                    ...
                    'total_lots': int(lots_to_sqf),
                    'lot_size': correct_lot_size,
        ...
        all_chunks = generate_chunked_orders(
            trade_uid_prefix=f"P{trade_uid}",
            legs_data=legs_data_for_batching,
            base_lots_for_trade=base_lots_for_trade,
            max_order_qty=max_order_qty,
        )
        ...
        for chunk in all_chunks:
            for order in chunk:
                if order.get('action', '').upper() == 'BUY':
                    order['limit_order_buffer'] = buy_buffer
                    order['buy_buffer'] = buy_buffer
                else:
                    order['limit_order_buffer'] = sell_buffer
                    order['sell_buffer'] = sell_buffer
        ...
        for chunk_idx, chunk_orders in enumerate(all_chunks, 1):
            ...
            for order in chunk_orders:
                if float(order.get('expected_price') or 0) <= 0:
                    try:
                        from market_data import get_ltp
                        fresh_ltp = await get_ltp(order['token'], order.get('exchange_segment'))
                        if fresh_ltp and fresh_ltp > 0:
                            order['expected_price'] = float(fresh_ltp)
                    except Exception:
                        pass

            invalid_orders = [o for o in chunk_orders if float(o.get('expected_price') or 0) <= 0]
            if invalid_orders:
                for bad in invalid_orders:
                    logger.error(
                        f"❌ Terminal pricing failure for {bad.get('uid')} "
                        f"(token={bad.get('token')}). Missing expected_price before execute_batch."
                    )
                chunk_orders = [o for o in chunk_orders if float(o.get('expected_price') or 0) > 0]

            if not chunk_orders:
                logger.error(f"❌ No executable orders remain in PARTIAL SQF chunk {chunk_idx} after pricing validation.")
                psqf_aborted = True
                break

            logger.info(f"⚡ Executing PARTIAL SQF chunk {chunk_idx}/{len(all_chunks)} with {len(chunk_orders)} orders.")
            # ============================================================
            # PARTIAL_EXIT_AT_STRADDLE_PRICE_GUARD_V2
            #
            # Same exit_at_straddle rule for the partial-square-off path.
            # ============================================================
            while True:
                exit_target = (
                    (straddle_data.get("config") or {})
                ...
                if exit_price_allowed:
                    logger.info(
                        f"[{trade_uid}] EXIT PRICE GUARD PASS | "
                        f"PARTIAL SQF chunk {chunk_idx}/{len(all_chunks)} "
                        f"may be submitted."
                    )
                    break
                ...
                if failed_order_info.get('status') == 'REEXECUTE_NEEDED':
                    ...
                    if original_order_uid:
                        ...
                        if original_order_data:
                            ...
                            if not order_for_re_execution.get('expected_price') or float(order_for_re_execution.get('expected_price') or 0) <= 0:
                                src_match = next(
                                    (o for o in successful_in_chunk if o.get('uid') == original_order_uid),
                                    None
                                )
                                if src_match:
                                    seed_price = (
                                        src_match.get('expected_price')
                                        or src_match.get('price')
                                        or src_match.get('ltp')
                                        or src_match.get('limit_price')
                                    )
                                    if seed_price:
                                        order_for_re_execution['expected_price'] = float(seed_price)

                            order_for_re_execution['exchange_segment'] = order_for_re_execution.get(
                                'exchange_segment',
                                original_order_data.get('exchange_segment')
                            )
                            order_for_re_execution['product_type'] = order_for_re_execution.get(
                                'product_type',
                                original_order_data.get('product_type', 'MIS')
                            )

                            if 'limit_order_buffer' not in order_for_re_execution:
                                order_for_re_execution['limit_order_buffer'] = original_order_data.get('limit_order_buffer', 2.0)
                            if 'buy_buffer' not in order_for_re_execution:
                                order_for_re_execution['buy_buffer'] = original_order_data.get('buy_buffer', buy_buffer)
                            if 'sell_buffer' not in order_for_re_execution:
                                order_for_re_execution['sell_buffer'] = original_order_data.get('sell_buffer', sell_buffer)
                            ...
                            if old_uid and old_uid[0].isalpha():
                                ...
                                if next_char > 'z' and old_uid[0].islower():
                                    next_char = 'a'
                                order_for_re_execution['uid'] = f"{next_char}{old_uid[1:]}"
                            else:
                                order_for_re_execution['uid'] = f"{old_uid}R"[:20]
            ...
            if orders_to_reexecute_in_next_chunk:
                next_chunk_list_index = chunk_idx
                if next_chunk_list_index < len(all_chunks):
                    all_chunks[next_chunk_list_index] = (
                        orders_to_reexecute_in_next_chunk + all_chunks[next_chunk_list_index]
                    )
                    logger.info(f"Prepended {len(orders_to_reexecute_in_next_chunk)} orders to next chunk {chunk_idx + 1}.")
                else:
        ...
        if all_verified_fills and hasattr(state.db, 'insert_order'):
            ...
            if trade_uid in state.trade_fill_cache:
                del state.trade_fill_cache[trade_uid]
        ...
        logger.critical(f"🛑 Partial Square-off for {trade_uid} was cancelled abruptly. Reverting status to ACTIVE.")
        await asyncio.get_event_loop().run_in_executor(
            None, state.db.update_straddle_status, trade_uid, 'ACTIVE'
        )
        raise
...
async def square_off_by_trade_uid(trade_uid: str, reason: str = None) -> Optional[Dict]:
    ...
    try:
        ...
        if not straddle_data:
            logger.error(f"❌ Could not find straddle data for {trade_uid} in DB.")
            return {'success': False, 'error': 'Trade data not found in DB'}
        ...
        if hasattr(state, 'trade_processes') and trade_uid in state.trade_processes:
            ...
            if not process or not process.is_alive():
                logger.warning(f"Process for trade {trade_uid} is not alive. Falling back to main-process square-off.")
                getattr(state, 'local_process_refs', {}).pop(trade_uid, None)
                getattr(state, 'local_command_queues', {}).pop(trade_uid, None)
                state.trade_processes.pop(trade_uid, None)
            elif not command_q:
                logger.warning(f"Command queue for trade {trade_uid} is missing. Falling back to main-process square-off.")
            else:
        ...
        logger.critical(f"🛑 Square-off dispatch for {trade_uid} was cancelled.")
        raise

    except Exception as e:
        logger.error(f"❌ Square-off by trade UID failed: {e}", exc_info=True)
        return {
            'success': False,
            'error': str(e),
            'trade_uid': trade_uid,
